...
const averageOdds = Math.trunc(sum / odds.length);
...
// TODO: Task 1
// Solution 1: We start by first converting the Map into array using the spread operator, and use the values() inorder to only get values from the gameEvent
// const eventArr = [...gameEvents.values()];
// console.log(eventArr);
// Then We can convert the resulted array to a set so as we dont have duplicate values
// const events = new Set(eventArr);
// console.log(events);

// Solution 2: we can directly use the spread operator and the values() so as we can directly create a set/array with no duplicate
