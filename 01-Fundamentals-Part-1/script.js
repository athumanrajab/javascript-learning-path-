...
// Since ternary opertaor is an expression then can produce a value , this make it possible to store it into variable

const drink = age >= 23 ? "Wine": "Water";
console.log(drink);

// But also it can be used inside the {} of the template literal because it's an expression
...
// The challenge above can be done by using either ternary operator or if/else statement, but since the challenge is to use ternary operator then we can do it as follows
...
// By using if/else statement, the challenge can be done as follows
if (bill >= 50 && bill <= 300) {
    tip = 0.15 * bill;
} else {
    tip = 0.2 * bill;
